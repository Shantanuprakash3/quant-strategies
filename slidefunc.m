function R = slidefunc (FUN, W, V, windowmode, varargin) 
% SLIDEFUNC - apply function to a moving window over a vector
%
%   R = SLIDEFUNC(FUN, W, V) evaluates the function FUN to a moving window
...
%
%   Effectively SLIDEFUNC applies the function FUN to a moving window of
...
%   R = SLIDEFUNC(FUN, W, V, WINDOWMODE) denotes the type of windowing being
...
%          so that x0 equals x
%
%  R = SLIDEFUNC(FUN,W,V,WINDOWMODE, P1,P2,...) provides for aditional
...
%      V =  [1  4  3  3  3  2  9  8] ;
%      R = slidefunc(myfun,3, V) 
%      % -> [3  3  1  0  1  7  7  1]
%
%   Example 4) Mimick cumsum
%      V = 1:10 ;
%      R = slidefunc(@sum, numel(V), V, 'backward') 
%      isequal(R,cumsum(V))
%
%   Example 5) Inverse cumprod ignoring zeros
%      V = [1:3 0 5:8] ;
%      myfun = inline('prod(x(x~=0))','x') ;
%      R = slidefunc(myfun, numel(V), V, 'forward') 
%
...
%      isoutlier = slidefunc(@(V,N) abs(V(N)-mean(V)) > 1.5*std(V), 2*N+1, V, [] ,N+1) 
...
% Some timings:
%   V = rand(10000,1) ; N = 3 ; tic ; R = slidefunc(@max,N, V) ; toc ;
%   % -> elapsed_time = 0.1870
%   V = rand(10000,1) ; N = 100 ; tic ; R = slidefunc(@max,N, V) ; toc ;
%   % -> elapsed_time = 0.3440